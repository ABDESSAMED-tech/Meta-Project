...
public class TaquinNode {
		/**
                 * dans cett partie on définie tout les information qui concerne un Etat données avec les  
                 * structeur de données utiliser dans les algorithme de recherche 
                 *
                 * */
        private int IState;
	private  int[][]  state = new int [3][3];
	private List<TaquinNode> children;
	...
	public TaquinNode(int [][] state) {
		this.state = state; // l'etat 
		this.depth = 1; // le profondeur de l'etat
		this.children = new ArrayList<TaquinNode>(); //les fils de  node
		this.parent = null;
		this.cost = 0;
		this.maxCost = 0;
		this.stringState = stringTaquin();
		this.direction = null;
		for(int i=0; i<=2; i++) {
			for(int j=0; j<=2; j++) {
				if(state[i][j]==0) {
	...
	public void setIStat(){
           String s="";
            ...
            this.IState= Integer.parseInt(s);
        }
        public int getIstat(){
            return this.IState;
        }
	public String stringTaquin() {   //pour l'affichage de l'etat 
	...
	public void addChild(TaquinNode child) { //adding a Child to the node
	...
	public void setParent(TaquinNode parent) { //setting the Parent of the node
		this.parent = parent;
	}
	...
	public TaquinNode getParent() {  //getting the Parent of the node
		return parent;
	}
	...
	public List<TaquinNode> getChildren(){ //getting the children
		return children;
	}
	public void setChildren(List<TaquinNode> childrens) { //setting the children
		this.children =  childrens;
	}
	
	public TaquinNode createChild(int a, int b) {      //creating the child or possible states from current node
		...
		int cost = state[a][b]; 
		temp[a][b] = 0;
		TaquinNode child = new TaquinNode(temp);
	...
	public boolean isGaol() {				//checking if node is goal node
		...
		TaquinNode goalNode = new TaquinNode(goal);
	...
	public boolean equals(Object object ) {    //equals for HashMap
		...
		if(!(object instanceof TaquinNode)) {
			return false;
		}
		TaquinNode check = (TaquinNode) object;
		
		return check.getString().equals(this.getString());
	}
	
	@Override
	public int hashCode() {	//Hashcode generated from String version of Taquin
		int result = 17;
	...
	public TaquinNode duplicateNode() {  //create duplicate Node
		TaquinNode dup = new TaquinNode(this.getMatrix());
