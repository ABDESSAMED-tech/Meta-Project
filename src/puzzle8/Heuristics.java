...
public class Heuristics {
	...
	public int numCorPos(TaquinNode node) {
	...
	public int manhattan(TaquinNode node) {   //second heuristic which uses a goal state to help determined how far argument node tiles are from desired position
		int [][]goal = {{1,2,3},{8,0,4},{7,6,5}};
		int result = 0;
		int [][]state = node.getMatrix();
		for(int i=0; i<state.length; i++) {
